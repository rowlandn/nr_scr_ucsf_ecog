...
% 091117 JR Use rawDataPath if outputPath does not exist. 
%           Now the *.tev file is found in the rawDataPath 
switch computer
    case 'PCWIN'
    case 'PCWIN64'
    otherwise
...
for k=1:length(chans2Convert)
    ...
    e=invoke (TT,'SetGlobals', ['Options = ALL; WavesMemLimit = 1.4e9;Channel=' num2str(chans2Convert(k)) '; T1=0; T2=' num2str(totaltime)]);
    ...
    disp(['saving ' outputPrefix num2str(chans2Convert(k)) ' ' num2str(k) '/' num2str(length(chans2Convert))]);
    blockNum=str2num(outputPrefix(end))
    writehtk(sprintf('%s%d.htk',outputPrefix,k),data, params.sampFreq,(blockNum-1)*64+k);
    sprintf('%s%d.htk',outputPrefix,k)
    %(blockNum-1)*64+k
    ...
    %save([outputPath filesep outputPrefix num2str(chans2Convert(k))],'data','params');

    else
        disp(['No data found for channel: ' num2str(chans2Convert(k)) '. Skipping!']);
        disp(['Run params=ecogTDTgetBlockInfo(''' rawDataPath ''',''' blockName ''') to get infos about the tank contents.']); 
    end
end
