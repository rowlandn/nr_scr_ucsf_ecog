function nr_spect(sum_filedirnm)
% Saves spectrogram data for later plotting
...
% Revised by NCR (11/12/2013)


%% parse subject and load data
load(sum_filedirnm)
...
if n_ecog == 5
    prep_onset = ecog.prep_time(trials_ok)/Fs;
elseif n_ecog == 28
...
if PRE<BL(1)
    ...
    last = int32((PRE-BL(2))/t_res);
    % to plot A with colors representing the log10 of power, uncomment this line:
    A2plot_on = log10(S_mag_mean);
    % to plot A with colors representing raw data values, uncomment this line:
    % A2plot_on = S_move_mag_mean;
    for i = 1:n_data_ch
        for j = 1:nfchans
            bl = A2plot_on(j,first:last,i);
            blmean = mean(bl);
            A2plot_on(j,:,i) = A2plot_on(j,:,i)/blmean; 
        end
    end
end
...
% else
save(['spec_',sbj],'A2plot_rp','A2plot_on','A2plot_off','faxis','taxis','move_onset','move_offset','PRE','POST');
